...
class SvgMap {
    ...
    pushMapElement(...items) {
        this.mapElements.push(...items);
    }
    ...
    latLongToXYToRef(lat, long, ref) {
        if (this.centerCoordinates.long > 90) {
            if (long < -90) {
                long += 360;
            }
        }
        else if (this.centerCoordinates.long < -90) {
            if (long > 90) {
                long -= 360;
            }
        }
        let x = (long - this.centerCoordinates.long) / this._angularWidth * 1000;
    ...
    coordinatesToXYToRef(coordinates, ref) {
        this.latLongToXYToRef(coordinates.lat, coordinates.long, ref);
    }
    latLongToXYToRefForceCenter(lat, long, ref, forcedCenterCoordinates) {
        if (forcedCenterCoordinates.long > 90) {
            if (long < -90) {
                long += 360;
            }
        }
        else if (forcedCenterCoordinates.long < -90) {
            if (long > 90) {
                long -= 360;
            }
        }
        let forcedAngularWidth = this._NMWidth / 60 / Math.cos(forcedCenterCoordinates.lat * Avionics.Utils.DEG2RAD);
        let x = (long - forcedCenterCoordinates.long) / forcedAngularWidth * 1000;
    ...
    coordinatesToXYToRefForceCenter(coordinates, ref, forcedCenterCoordinates) {
        this.latLongToXYToRefForceCenter(coordinates.lat, coordinates.long, ref, forcedCenterCoordinates);
    }
    XYToCoordinates(xy) {
