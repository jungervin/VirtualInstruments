class FlightPlanManager {
    ...
    update(_deltaTime) {
        ...
        this._activeWaypointHasChanged = false;
        let gpsActiveWaypointIndex = this._activeWaypointIndex;
        ...
        if (this._directToCancelDistance == undefined || !isFinite(this._directToCancelDistance)) {
            this.updateDirectToCancelDistance();
        }
    ...
    _loadWaypoints(data, currentWaypoints, callback) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (currentWaypoints[ii] &&
                ...
                v.ident = waypointData.ident;
                v.infos.ident = waypointData.ident;
    ...
    updateCurrentApproachWaypoints(callback = EmptyCallback.Void, log = false) {
    ...
    async asyncUpdateCurrentApproachWaypoints(log = false) {
        return new Promise(resolve => {
            this.updateCurrentApproachWaypoints(resolve, log);
        });
    }
    updateCurrentApproach(callback = () => { }, log = false) {
        let t0 = performance.now();
        this.updateCurrentApproachWaypoints(EmptyCallback.Void, log);
    ...
    clearFlightPlanByIndex(index, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
        Coherent.call("CLEAR_FLIGHT_PLAN_BY_INDEX", index).then(() => {
            if (doUpdateFlightPlan) {
                this.updateFlightPlanByIndex(() => {
                    callback();
                }, index);
            }
            else {
    ...
    async asyncClearFlightPlanByIndex(index, doUpdateFlightPlan = true) {
        return new Promise(resolve => {
            this.clearFlightPlanByIndex(index, resolve, doUpdateFlightPlan);
        });
    }
    ...
    getPreviousActiveWaypoint() {
        if (this.isActiveApproach()) {
            ...
            return approachWaypoints[this._activeWaypointIndex - 1];
        }
        let waypoints = this.getWaypoints();
        if (!waypoints) {
            return;
        }
        return waypoints[this._activeWaypointIndex - 1];
    }
    getActiveWaypoint(useCorrection = false) {
        ...
        let index = this._activeWaypointIndex;
        if (this.isActiveApproach()) {
            return this.getApproachWaypoints()[index];
        }
        else {
            return this.getWaypoints()[index];
        }
    }
    ...
    getWaypointsWithAltitudeConstraints() {
        ...
        for (let i = 0; i < this._waypoints[0].length; i++) {
            ...
            if (wp && wp.ident != "" && wp.legAltitudeDescription >= 1 && wp.legAltitude1 < 20000) {
        ...
        for (let i = 0; i < approachWaypoints.length; i++) {
            ...
            if (apprWp && apprWp.ident != "" && apprWp.legAltitudeDescription >= 1 && apprWp.legAltitude1 < 20000) {
    ...
    addWaypoint(icao, index = Infinity, callback = EmptyCallback.Void, setActive = true, doUpdateFlightPlan = true) {
        if (index === Infinity) {
    ...
    async asyncAddWaypoint(icao, index = Infinity, setActive = true, doUpdateFlightPlan = true) {
        return new Promise(resolve => {
            this.addWaypoint(icao, index, resolve, setActive, doUpdateFlightPlan);
        });
    }
    addCustomWaypoint(ident, index, latitude, longitude, setActive = true, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
    ...
    async asyncAddCustomWaypoint(ident, index, latitude, longitude, setActive = true, doUpdateFlightPlan = true) {
        return new Promise(resolve => {
            this.addCustomWaypoint(ident, index, latitude, longitude, setActive, resolve, doUpdateFlightPlan);
        });
    }
    setWaypointLatLon(index, latitude, longitude, setActive = true, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
    ...
    setWaypointLatLonAlt(index, latitude, longitude, altitude, setActive = true, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
    ...
    async asyncSetWaypointLatLonAlt(index, latitude, longitude, altitude, setActive = true, doUpdateFlightPlan = true) {
        return new Promise(resolve => {
            this.setWaypointLatLonAlt(index, latitude, longitude, altitude, setActive, resolve, doUpdateFlightPlan);
        });
    }
    setWaypointAltitude(altitude, index, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
    ...
    async asyncSetWaypointAltitude(altitude, index, doUpdateFlightPlan = true) {
        return new Promise(resolve => {
            this.setWaypointAltitude(altitude, index, resolve, doUpdateFlightPlan);
        });
    }
    setWaypointAdditionalData(index, key, value, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
    ...
    removeWaypoint(index, thenSetActive = false, callback = EmptyCallback.Void, doUpdateFlightPlan = true) {
        if (index == 0 && SimVar.GetSimVarValue("L:Glasscockpits_FPLHaveOrigin", "boolean")) {
            ...
            Coherent.call("REMOVE_DESTINATION", index, thenSetActive).then(() => {
                ...
                if (doUpdateFlightPlan) {
                    this.updateFlightPlan(() => {
                        this.updateCurrentApproach(callback);
                    });
                }
                else {
                    callback();
    ...
    async asyncRemoveWaypoint(index, thenSetActive = false, doUpdateFlightPlan = true) {
        return new Promise(resolve => {
            this.removeWaypoint(index, thenSetActive, resolve, doUpdateFlightPlan);
        });
    }
    ...
    activateDirectToFromLatLonAlt(latitude, longitude, altitude, callback = EmptyCallback.Void, ident = "") {
        Coherent.call("ACTIVATE_DIRECT_TO_FROM_LATLONALT", latitude, longitude, altitude, ident).then(() => {
            this.updateFlightPlan(callback);
        });
    }
    async asyncActivateDirectToFromLatLonAlt(latitude, longitude, altitude, ident = "") {
        return new Promise(resolve => {
            this.activateDirectToFromLatLonAlt(latitude, longitude, altitude, resolve, ident);
        });
    }
    ...
    setDirectToCancelDistance(distance, callback = EmptyCallback.Boolean) {
        Coherent.call("SET_DIRECT_TO_CANCEL_DISTANCE", distance).then(() => {
            let attempts = 0;
            let checkTrueDirectToCancelDistance = () => {
                Coherent.call("GET_DIRECT_TO_CANCEL_DISTANCE").then((value) => {
                    attempts++;
                    if (value === distance) {
                        this._directToCancelDistance = value;
                        return callback(true);
                    }
                    else {
                        if (attempts < 10) {
                            console.log("setDirectToCancelDistance : Values mistmatch, retrying");
                            this.instrument.requestCall(checkTrueDirectToCancelDistance);
                            return;
                        }
                        else {
                            console.log("setDirectToCancelDistance : Values mistmatched too long, aborting");
                            return callback(false);
                        }
                    }
                });
            };
            checkTrueDirectToCancelDistance();
        });
    }
    async asyncSetDirectToCancelDistance(distance) {
        return new Promise(resolve => {
            this.setDirectToCancelDistance(distance, resolve);
        });
    }
    updateDirectToCancelDistance() {
        Coherent.call("GET_DIRECT_TO_CANCEL_DISTANCE").then((distance) => {
            this._directToCancelDistance = distance;
        });
    }
